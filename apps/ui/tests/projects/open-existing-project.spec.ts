 ...
 * 2. The .dmaker directory is initialized if it doesn't exist
...
test.describe('Open Project', () => {
  ...
  test('should open an existing project directory from recent projects', async ({ page }) => {
    ...
    // Wait for a project to be set as current and visible in the top navigation bar
    ...
    if (targetProjectName) {
      ...
      await expect(page.getByTestId('project-dropdown-trigger')).toContainText(targetProjectName, {
        timeout: 15000,
      });
    }
    ...
    if (targetProjectName === projectName) {
      // Verify .dmaker directory was created (initialized for the first time)
      // Use polling since file creation may be async
      const dmakerDir = path.join(projectPath, '.dmaker');
      await expect(async () => {
        expect(fs.existsSync(dmakerDir)).toBe(true);
      }).toPass({ timeout: 10000 });

      // Verify the required structure was created by initializeProject:
      // - .dmaker/categories.json
      // - .dmaker/features directory
      // - .dmaker/context directory
      const categoriesPath = path.join(dmakerDir, 'categories.json');
      await expect(async () => {
        expect(fs.existsSync(categoriesPath)).toBe(true);
      }).toPass({ timeout: 10000 });

      // Verify subdirectories were created
      expect(fs.existsSync(path.join(dmakerDir, 'features'))).toBe(true);
      expect(fs.existsSync(path.join(dmakerDir, 'context'))).toBe(true);
