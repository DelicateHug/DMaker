  ...
  await page.addInitScript(
    ({
      ...
      if (opts?.workspaceDir) {
        localStorage.setItem('dmaker:lastProjectDir', opts.workspaceDir);
      }
      ...
      sessionStorage.setItem('dmaker-test-welcome-view', 'true');

      // Override currentProject after a short delay to ensure it happens after settings hydration
      setTimeout(() => {
        const storage = localStorage.getItem('dmaker-storage');
        if (storage) {
          try {
            const state = JSON.parse(storage);
            if (state.state && sessionStorage.getItem('dmaker-test-welcome-view') === 'true') {
              state.state.currentProject = null;
              state.state.currentView = 'welcome';
              localStorage.setItem('dmaker-storage', JSON.stringify(state));
            }
          } catch {
            // Ignore parse errors
          }
  ...
  await page.addInitScript(
    ({ featureId, contextContent }: { featureId: string; contextContent: string }) => {
      ...
      (window as any).__mockContextFile = {
        featureId,
        path: `/mock/test-project/.dmaker/features/${featureId}/agent-output.md`,
...
export async function setupFirstRun(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    localStorage.removeItem('dmaker-setup');
    localStorage.removeItem('dmaker-storage');
