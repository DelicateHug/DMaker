...
/**
 * Gets the default Documents/DMaker directory path
 * @returns Promise resolving to Documents/DMaker path, or null if unavailable
 */
async function getDefaultDocumentsPath(): Promise<string | null> {
  try {
    ...
    if (typeof window !== 'undefined' && (window as any).electronAPI?.getPath) {
      ...
      return joinPath(documentsPath, 'DMaker');
    }
 ...
 * 3. Documents/DMaker (if ALLOWED_ROOT_DIRECTORY is not set)
