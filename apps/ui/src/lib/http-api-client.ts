...
const getServerUrl = (): string => {
  ...
  const port = import.meta.env.DEV ? 3019 : 3009;
...
export class HttpApiClient implements ElectronAPI {
  ...
  github: GitHubAPI = {
    ...
    reopenIssue: (
      ...
      this.post('/api/github/reopen-issue', { projectPath, issueNumber }),
    closeIssue: (
      projectPath: string,
      issueNumber: number
    ): Promise<{ success: boolean; error?: string }> =>
      this.post('/api/github/close-issue', { projectPath, issueNumber }),
  };
  ...
  settings = {
    ...
    discoverAgents: (
      ...
      error?: string;
    }> => this.post('/api/settings/agents/discover', { projectPath, sources, projects }),
  };
  ...
  agents = {
    save: (data: {
      ...
      this.post('/api/agents/save', data),

    delete: (filePath: string): Promise<{ success: boolean; error?: string }> =>
      this.post('/api/agents/delete', { filePath }),
    ...
    generate: (data: {
      ...
      error?: string;
      rawResponse?: string;
    }> => this.post('/api/agents/generate', data),
    ...
    evaluate: (data: {
      ...
      error?: string;
      rawResponse?: string;
    }> => this.post('/api/agents/evaluate', data),
  };
