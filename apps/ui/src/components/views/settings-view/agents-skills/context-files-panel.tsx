/**
 * Context Files Panel - Interactive manager for .dmaker/context/ files
 *
 * Discovers context files from all projects with subfolder support.
 */
...
import { useContextFiles, type ContextFile } from './hooks/use-context-files';

export type ContextFilesData = ReturnType<typeof useContextFiles>;
import { ContextFileCard } from './context-file-card';
import { ContextFileEditorDialog } from './context-file-editor-dialog';

function ContextFolderGroup({
  path: folderPath,
  files: folderFiles,
  onEdit,
  onDelete,
...
function groupByFolder(filesList: ContextFile[]) {
  const folderMap = new Map<string, ContextFile[]>();
...
export function ContextFilesPanel({ data }: { data: ContextFilesData }) {
  const {
    files,
    isLoading,
    hasProject,
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<ContextFile | null>(null);

  // Rename dialog state
  const [renameTarget, setRenameTarget] = useState<ContextFile | null>(null);
  ...
  const handleDeleteFile = (file: ContextFile) => {
  ...
  const handleRenameFile = (file: ContextFile) => {
  ...
  const renderFileList = (folderFiles: ContextFile[]) =>
    folderFiles.map((file) => (
      <ContextFileCard
        key={file.path}
        file={file}
    ...
    <>
      <div className="rounded-xl border border-border/30 bg-muted/30 p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-foreground/80">Context Files</span>
              {files.length > 0 && (
        ...
        <p className="text-xs text-muted-foreground">
          Files in <code className="text-xs bg-muted px-1 rounded">.dmaker/context/</code> are
          always loaded into agent prompts. Use this for project-specific rules, coding standards,
          or context that should apply to all features.
        </p>
        ...
        {!hasProject ? (
          <div className="text-center py-4 text-muted-foreground">
            ...
            <p className="text-xs">Select a project to manage context files</p>
          </div>
        ) : files.length === 0 && !isLoading ? (
          <div className="text-center py-4 text-muted-foreground border border-dashed border-border/50 rounded-lg">
            <FileText className="w-8 h-8 mx-auto mb-2 opacity-30" />
            <p className="text-xs font-medium">No context files yet</p>
            <p className="text-xs mt-1">Create a file to add project-specific rules for agents</p>
          </div>
        ) : (
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card border border-border rounded-xl p-6 max-w-sm mx-4 shadow-xl">
            <h3 className="font-semibold text-base">Delete Context File</h3>
      ...
      <Dialog open={!!renameTarget} onOpenChange={(open) => !open && setRenameTarget(null)}>
        <DialogContent>
          ...
          <div className="py-4">
            <div className="space-y-2">
              <Label htmlFor="rename-context-file">File Name</Label>
              <Input
                id="rename-context-file"
                value={renameValue}
