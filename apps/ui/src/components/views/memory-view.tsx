...
export function MemoryView() {
  ...
  // Tier system file helpers
  const TIER_FILES = ['_memory-small.md', '_memory-medium.md', '_memory-high.md'];
  const MASTER_FILE = '_master-memory.md';

  const isTierFile = (name: string) => TIER_FILES.includes(name);
  const isMasterFile = (name: string) => name === MASTER_FILE;
  const isAutoGenerated = (name: string) => isTierFile(name) || isMasterFile(name);

  const getTierLabel = (name: string): string => {
    if (name === '_memory-small.md') return 'Small';
    if (name === '_memory-medium.md') return 'Medium';
    if (name === '_memory-high.md') return 'High';
    return '';
  };

  const getTierVariant = (name: string): 'info' | 'warning' | 'success' => {
    if (name === '_memory-small.md') return 'info';
    if (name === '_memory-medium.md') return 'warning';
    if (name === '_memory-high.md') return 'success';
    return 'info';
  };

  // Group files into sections
  const { tierFiles, masterFiles, categoryFiles } = useMemo(() => {
    const tier: MemoryFile[] = [];
    const master: MemoryFile[] = [];
    const category: MemoryFile[] = [];

    for (const file of memoryFiles) {
      if (isTierFile(file.name)) tier.push(file);
      else if (isMasterFile(file.name)) master.push(file);
      else category.push(file);
    }

    // Sort tier files in order: small, medium, high
    tier.sort((a, b) => TIER_FILES.indexOf(a.name) - TIER_FILES.indexOf(b.name));

    return { tierFiles: tier, masterFiles: master, categoryFiles: category };
  }, [memoryFiles]);
  ...
  // Regenerate memory tiers
  const handleRegenerateTiers = async () => {
    if (!currentProject || isRegenerating) return;

    setIsRegenerating(true);
    try {
      await apiPost('/api/memory/regenerate-tiers', {
    ...
    <div className="flex-1 flex flex-col overflow-hidden content-bg" data-testid="memory-view">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden lg:flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={handleRegenerateTiers}
              disabled={isRegenerating || categoryFiles.length === 0}
              data-testid="regenerate-tiers-button"
            >
              {isRegenerating ? (
              ...
              {isRegenerating ? 'Regenerating...' : 'Regenerate Tiers'}
            </Button>
            <Button
              variant="outline"
              size="sm"
      ...
      >
        <Button
          ...
          onClick={() => {
            handleRegenerateTiers();
            setShowActionsPanel(false);
          }}
          disabled={isRegenerating || categoryFiles.length === 0}
          data-testid="regenerate-tiers-button-mobile"
        >
          {isRegenerating ? (
            <RefreshCw className="w-4 h-4 mr-2 animate-spin" />
          ) : (
            ...
            {memoryFiles.length === 0 ? (
              ...
              <div className="space-y-1">
                    ...
                    {tierFiles.map((file) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="truncate text-sm block">{getTierLabel(file.name)}</span>
                        </div>
                        <Badge variant={getTierVariant(file.name)} size="sm">
                          {getTierLabel(file.name)}
                        </Badge>
                      </div>
                    ))}
                  </>
                  ...
                  <>
                    ...
                    {masterFiles.map((file) => (
                      ...
                      >
                        <Lock className="w-3.5 h-3.5 flex-shrink-0 text-muted-foreground/60" />
                        <div className="min-w-0 flex-1">
                          <span className="truncate text-sm block">Master Memory</span>
                        </div>
                      </div>
                    ))}
                  </>
                ...
                {categoryFiles.length > 0 && (
                  <>
                    {(tierFiles.length > 0 || masterFiles.length > 0) && (
                      <div className="flex items-center gap-1.5 px-2 pt-3 pb-1">
          ...
          {selectedFile ? (
            <>
              ...
              <div className="flex items-center justify-between p-3 border-b border-border bg-card">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm font-medium truncate">
                    {isTierFile(selectedFile.name)
                      ? `${getTierLabel(selectedFile.name)} Tier`
                      : isMasterFile(selectedFile.name)
                        ? 'Master Memory'
                        : selectedFile.name}
                  </span>
                  {isAutoGenerated(selectedFile.name) && (
                  ...
                  {isTierFile(selectedFile.name) && (
                    <Badge variant={getTierVariant(selectedFile.name)} size="sm">
                      {getTierLabel(selectedFile.name)}
                    </Badge>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-hidden p-4">
                {isPreviewMode || isAutoGenerated(selectedFile.name) ? (
