/**
 * RequestCache - Generic in-memory cache with request deduplication and SWR support
 *
 * A general-purpose caching layer for async operations (HTTP requests, CLI calls, etc.)
...
export interface RequestCacheOptions {
   ...
   * (default: same as defaultTtl, so stale data lives for 2× TTL total)
   */
  swrTtl?: number;
 ...
 * @example
 * ```ts
 * const modelCache = new RequestCache<string, Model[]>({ defaultTtl: 300_000 });
 *
 * const models = await modelCache.getOrSet('all-models', async () => {
...
export class RequestCache<K = string, V = unknown> {
   ...
   * @example
   * ```ts
   * // Invalidate all entries whose key starts with "models:"
   * cache.invalidateBy(key => key.startsWith('models:'));
   * ```
   */
