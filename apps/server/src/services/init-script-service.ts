 ...
 *
 * Runs the .dmaker/worktree-init.sh script after worktree creation.
...
export class InitScriptService {
  ...
  getInitScriptPath(projectPath: string): string {
    return path.join(projectPath, '.dmaker', 'worktree-init.sh');
  }
  ...
  async runInitScript(options: InitScriptOptions): Promise<void> {
    ...
    const safeEnv: Record<string, string> = {
      // DMaker-specific variables
      DMAKER_PROJECT_PATH: projectPath,
      DMAKER_WORKTREE_PATH: worktreePath,
      DMAKER_BRANCH: branch,
