...
export class TerminalService extends EventEmitter {
  ...
  async createSession(options: TerminalOptions = {}): Promise<TerminalSession | null> {
    ...
    // First, create a clean copy of process.env excluding DMaker-specific variables
    // that could pollute user shells (e.g., PORT would affect Next.js/other dev servers)
    const dmakerEnvVars = ['PORT', 'DATA_DIR', 'DMAKER_API_KEY', 'NODE_PATH'];
    const cleanEnv: Record<string, string> = {};
    for (const [key, value] of Object.entries(process.env)) {
      if (value !== undefined && !dmakerEnvVars.includes(key)) {
        cleanEnv[key] = value;
      }
    }
