...
import { createLogger, isAbortError } from '@dmaker/utils';
import { spawnJSONLProcess, execInWsl } from '@dmaker/platform';
...
export class CursorProvider extends CliProvider {
  ...
  /**
   * Convert Cursor event to DMaker ProviderMessage format
  ...
  async *executeQuery(options: ExecuteOptions): AsyncGenerator<ProviderMessage> {
    ...
    if (options.mcpServers && Object.keys(options.mcpServers).length > 0) {
      ...
      logger.warn(
        `MCP servers configured (${serverCount}) but not yet supported by Cursor CLI in DMaker. ` +
    ...
    // Debug: log raw events when DMAKER_DEBUG_RAW_OUTPUT is enabled
