# Contributing to DMaker

Thank you for your interest in contributing to DMaker! We're excited to have you join our community of developers building the future of autonomous AI development.

DMaker is an autonomous AI development studio that provides a Kanban-based workflow where AI agents implement features in isolated git worktrees. Whether you're fixing bugs, adding features, improving documentation, or suggesting ideas, your contributions help make this project better for everyone.

This guide will help you get started with contributing to DMaker. Please take a moment to read through these guidelines to ensure a smooth contribution process.

## Contribution License Agreement

**Important:** By submitting, pushing, or contributing any code, documentation, pull requests, issues, or other materials to the DMaker project, you agree to assign all right, title, and interest in and to your contributions, including all copyrights, patents, and other intellectual property rights, to the Core Contributors of DMaker. This assignment is irrevocable and includes the right to use, modify, distribute, and monetize your contributions in any manner.
...
- [Contributing to DMaker](#contributing-to-dmaker)
...
Before contributing to DMaker, ensure you have the following installed on your system:
...
1. **Fork the repository** on GitHub
   - Navigate to [https://github.com/DelicateHug/DMaker](https://github.com/DelicateHug/DMaker)
...
2. **Clone your fork locally**
   ...
   ```bash
   git clone https://github.com/YOUR_USERNAME/dmaker.git
   cd dmaker
   ```
...
3. **Add the upstream remote** to keep your fork in sync
   ...
   ```bash
   git remote add upstream https://github.com/DelicateHug/DMaker.git
   ```
...
4. **Verify remotes**
   ...
   git remote -v
   # Should show:
   # origin    https://github.com/YOUR_USERNAME/dmaker.git (fetch)
   # origin    https://github.com/YOUR_USERNAME/dmaker.git (push)
   # upstream  https://github.com/DelicateHug/DMaker.git (fetch)
   # upstream  https://github.com/DelicateHug/DMaker.git (push)
   ```
...
DMaker is organized as an npm workspace monorepo:
...
├── libs/
│   ├── @dmaker/types/            # Shared TypeScript types
│   ├── @dmaker/utils/            # Utility functions
│   ├── @dmaker/prompts/          # AI prompt templates
│   ├── @dmaker/platform/         # Platform abstractions
│   ├── @dmaker/model-resolver/   # AI model resolution
│   ├── @dmaker/dependency-resolver/ # Dependency management
│   └── @dmaker/git-utils/        # Git operations
├── docs/                # Documentation
...
- Always import from `@dmaker/*` shared packages, never use relative paths to `libs/`
...
DMaker uses **Release Candidate (RC) branches** for all development work. Understanding this workflow is essential before contributing.
...
3. **Important:** Set the base repository to `DelicateHug/DMaker` and the base branch to the **current RC branch** (e.g., `v0.11.0rc`), not `main`
...
This PR adds dark mode support to the DMaker UI.
...
DMaker uses automated tooling to enforce code style. Run `npm run format` to format code and `npm run lint` to check for issues. Pre-commit hooks automatically format staged files before committing.

---

## Testing Requirements

Testing helps prevent regressions. DMaker uses **Playwright** for end-to-end testing and **Vitest** for unit tests.
...
npm test --workspace=@dmaker/ui -- tests/example.spec.ts
```
...
DMaker uses **GitHub Actions** for continuous integration. Every pull request triggers automated checks.
...
| **Package Tests** | Runs tests for shared `@dmaker/*` packages  |
| **Server Tests**  | Runs server unit tests with coverage        |

#### CI Testing Environment

For CI environments, DMaker supports a mock agent mode:

```bash
# Enable mock agent mode for CI testing
DMAKER_MOCK_AGENT=true npm run test
```
...
2. **Try the latest version** - Make sure you're running the latest version of DMaker
...
We welcome ideas for improving DMaker! Here's how to submit a feature request:

#### Before Requesting

1. **Check existing issues** - Your idea may already be proposed or in development
2. **Consider scope** - Think about whether the feature fits DMaker's mission as an autonomous AI development studio
...
Thank you for contributing to DMaker!
