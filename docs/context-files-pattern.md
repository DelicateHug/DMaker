...
This document describes how context files work in DMaker and how to use them in agent prompts.

## Overview

Context files are user-defined documents stored in `.dmaker/context/` that provide project-specific rules, conventions, and guidelines for AI agents. They are automatically loaded and prepended to agent prompts.

## Directory Structure

```
{projectPath}/.dmaker/context/
...
The `loadContextFiles` function from `@dmaker/utils` provides a unified way to load context files:

```typescript
import { loadContextFiles } from '@dmaker/utils';
...
**Path:** `/path/to/project/.dmaker/context/CLAUDE.md`
...
**Path:** `/path/to/project/.dmaker/context/CODE_QUALITY.md`
