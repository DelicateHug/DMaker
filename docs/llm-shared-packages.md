# DMaker Shared Packages - LLM Guide

This guide helps AI assistants understand how to use DMaker's shared packages effectively.

## Package Overview

DMaker uses a monorepo structure with shared packages in `libs/`:
...
**Use when:** You need type definitions for any DMaker concept.
...
```typescript
import { getEnhancementPrompt, isValidEnhancementMode } from '@dmaker/prompts';
...
**Use when:** You need to work with DMaker's directory structure or spawn processes.

**Import for:**

- `getDmakerDir(projectPath)` - Get .dmaker directory
- `getFeaturesDir(projectPath)` - Get features directory
- `getFeatureDir(projectPath, featureId)` - Get specific feature directory
- `ensureDmakerDir(projectPath)` - Create .dmaker if needed
...
**Never import from:** `lib/dmaker-paths`, `lib/subprocess-manager`, `lib/security`
...
```typescript
import { resolveDependencies, areDependenciesSatisfied } from '@dmaker/dependency-resolver';
...
```typescript
import { isGitRepo, getGitRepositoryDiffs } from '@dmaker/git-utils';
...
import { createLogger, classifyError } from '@dmaker/utils';
import { resolveModelString, DEFAULT_MODELS } from '@dmaker/model-resolver';
import { areDependenciesSatisfied } from '@dmaker/dependency-resolver';
import { getFeatureDir } from '@dmaker/platform';
...
```typescript
import { getGitRepositoryDiffs, parseGitStatus } from '@dmaker/git-utils';
...
```typescript
// Import types from @dmaker/types
import type { Feature, ExecuteOptions } from '@dmaker/types';

// Import constants from @dmaker/types
import { CLAUDE_MODEL_MAP, DEFAULT_MODELS } from '@dmaker/types';

// Import utilities from @dmaker/utils
import { createLogger, classifyError } from '@dmaker/utils';

// Import prompts from @dmaker/prompts
import { getEnhancementPrompt, isValidEnhancementMode } from '@dmaker/prompts';

// Import platform utils from @dmaker/platform
import { getFeatureDir, ensureDmakerDir } from '@dmaker/platform';

// Import model resolution from @dmaker/model-resolver
import { resolveModelString } from '@dmaker/model-resolver';

// Import dependency resolution from @dmaker/dependency-resolver
import { resolveDependencies } from '@dmaker/dependency-resolver';

// Import git utils from @dmaker/git-utils
...
import { getFeatureDir } from '../lib/dmaker-paths';         // ❌
import { classifyError } from '../lib/error-handler';           // ❌

// DON'T define types that exist in @dmaker/types
interface Feature { ... }  // ❌ Use: import type { Feature } from '@dmaker/types';
```
...
- [ ] All `Feature` imports use `@dmaker/types`
- [ ] All `ExecuteOptions` imports use `@dmaker/types`
- [ ] All logger usage uses `@dmaker/utils`
- [ ] All prompt templates use `@dmaker/prompts`
- [ ] All path operations use `@dmaker/platform`
- [ ] All model resolution uses `@dmaker/model-resolver`
- [ ] All dependency checks use `@dmaker/dependency-resolver`
- [ ] All git operations use `@dmaker/git-utils`
...
- Types → `@dmaker/types`
- Logging/Errors/Utils → `@dmaker/utils`
- AI Prompts → `@dmaker/prompts`
- Paths/Security → `@dmaker/platform`
- Model Resolution → `@dmaker/model-resolver`
- Dependency Ordering → `@dmaker/dependency-resolver`
- Git Operations → `@dmaker/git-utils`
