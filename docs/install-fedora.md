# Installing DMaker on Fedora/RHEL

This guide covers installation of DMaker on Fedora, RHEL, Rocky Linux, AlmaLinux, and other RPM-based distributions.
...
1. Visit [GitHub Releases](https://github.com/DelicateHug/DMaker/releases)
2. Find the latest release and download the `.rpm` file:
   - Download: `DMaker-<version>-x86_64.rpm`
...
Install from GitHub releases URL without downloading first. Visit [releases page](https://github.com/DelicateHug/DMaker/releases) to find the latest version.

**Fedora:**

```bash
# Replace v0.11.0 with the actual latest version
sudo dnf install https://github.com/DelicateHug/DMaker/releases/download/v0.11.0/DMaker-0.11.0-x86_64.rpm
```
...
```bash
# Replace v0.11.0 with the actual latest version
sudo yum install https://github.com/DelicateHug/DMaker/releases/download/v0.11.0/DMaker-0.11.0-x86_64.rpm
```

## Running DMaker

After successful installation, launch DMaker:
...
DMaker stores configuration and cache in:

```
~/.dmaker/                # Project-specific data
~/.config/dmaker/         # Application configuration
~/.cache/dmaker/          # Cache and temporary files
```
...
```bash
cat /usr/share/applications/dmaker.desktop
```
...
If DMaker fails on SELinux-enforced systems:
...
# Check for denials
sudo ausearch -m avc -ts recent | grep dmaker
...
Create custom SELinux policy based on ausearch output. For support, see [GitHub Issues](https://github.com/DelicateHug/DMaker/issues).

### Port Conflicts

DMaker uses port 3008 for the internal server. If port is already in use:
...
Or configure DMaker to use different port (see Configuration section).
...
DMaker uses Chromium for rendering. GPU acceleration should work automatically on supported systems.
...
```bash
DISABLE_GPU_ACCELERATION=1 dmaker
```
...
2. Check process: `ps aux | grep dmaker`
3. Force quit if necessary: `killall dmaker`
...
```bash
sudo dnf remove dmaker
```

**RHEL/CentOS:**

```bash
sudo yum remove dmaker
```
...
To build DMaker from source on Fedora/RHEL:
...
**Automatic Updates:**
DMaker checks for updates on startup. Install available updates through notifications.

**Manual Update:**

```bash
# Fedora
sudo dnf update dmaker

# RHEL/CentOS
sudo yum update dmaker

# Or reinstall latest release
sudo dnf remove dmaker

# Download the latest .rpm from releases page
# https://github.com/DelicateHug/DMaker/releases
# Then reinstall with:
# sudo dnf install ./DMaker-<VERSION>-x86_64.rpm
```
...
- [GitHub Issues](https://github.com/DelicateHug/DMaker/issues) - Bug reports & feature requests
- [Discussions](https://github.com/DelicateHug/DMaker/discussions) - Questions & community
...
dmaker 2>&1 | tee dmaker.log

# SELinux status
getenforce

# Relevant system logs
sudo journalctl -xeu dmaker.service (if systemd service exists)
```
...
echo "ANTHROPIC_API_KEY=sk-ant-..." > ~/.config/dmaker/.env
...
1. Generate policy from audit logs: `ausearch -m avc -ts recent | grep dmaker`
...
```bash
chmod 600 ~/.config/dmaker/.env
chmod 700 ~/.dmaker/
chmod 700 ~/.config/dmaker/
```
