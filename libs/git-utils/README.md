...
Git operations and utilities for DMaker.
...
```typescript
import { parseGitStatus } from '@dmaker/git-utils';
import type { FileStatus } from '@dmaker/git-utils';
...
```typescript
import { listAllFilesInDirectory, generateDiffsForNonGitDirectory } from '@dmaker/git-utils';
...
```typescript
import { isGitRepo, getGitRepositoryDiffs, parseGitStatus } from '@dmaker/git-utils';
...
try {
  ...
  // Errors are logged via @dmaker/utils logger
...
- `@dmaker/types` - FileStatus type definition
- `@dmaker/utils` - Logger utilities

## Used By

- `@dmaker/server` - Git routes, worktree operations, feature context
