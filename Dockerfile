# DMaker Multi-Stage Dockerfile
# Single Dockerfile for both server and UI builds
# Usage:
#   docker build --target server -t dmaker-server .
#   docker build --target ui -t dmaker-ui .
...
LABEL dmaker.git.commit.sha="${GIT_COMMIT_SHA}"
...
RUN groupadd -o -g ${GID} dmaker && \
    useradd -o -u ${UID} -g dmaker -m -d /home/dmaker -s /bin/bash dmaker && \
    mkdir -p /home/dmaker/.local/bin && \
    mkdir -p /home/dmaker/.cursor && \
    chown -R dmaker:dmaker /home/dmaker && \
    chmod 700 /home/dmaker/.cursor

# Install Cursor CLI as the dmaker user
# Set HOME explicitly and install to /home/dmaker/.local/bin/
USER dmaker
ENV HOME=/home/dmaker
RUN curl https://cursor.com/install -fsS | bash && \
    echo "=== Checking Cursor CLI installation ===" && \
    ls -la /home/dmaker/.local/bin/ && \
    echo "=== PATH is: $PATH ===" && \
...
RUN mkdir -p /etc/profile.d && \
    echo 'export PATH="/home/dmaker/.local/bin:$PATH"' > /etc/profile.d/cursor-cli.sh && \
    chmod +x /etc/profile.d/cursor-cli.sh

# Add to dmaker's .bashrc for bash interactive shells
RUN echo 'export PATH="/home/dmaker/.local/bin:$PATH"' >> /home/dmaker/.bashrc && \
    chown dmaker:dmaker /home/dmaker/.bashrc

# Also add to root's .bashrc since docker exec defaults to root
RUN echo 'export PATH="/home/dmaker/.local/bin:$PATH"' >> /root/.bashrc
...
RUN mkdir -p /data /projects && chown dmaker:dmaker /data /projects
...
# The entrypoint script will switch to dmaker user before running the command
...
ENV PATH="/home/dmaker/.local/bin:${PATH}"
