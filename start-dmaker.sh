...
HISTORY_FILE="${HOME}/.dmaker_launcher_history"
MIN_TERM_WIDTH=70
MIN_TERM_HEIGHT=20
MENU_BOX_WIDTH=66
MENU_INNER_WIDTH=64
...
show_help() {
    cat << 'EOF'
DMaker TUI Launcher - Interactive development environment starter

USAGE:
  start-dmaker.sh [MODE] [OPTIONS]
...
EXAMPLES:
  start-dmaker.sh              # Interactive menu (development)
  start-dmaker.sh --production # Interactive menu (production)
...
HISTORY:
  Your last selected mode is remembered in: ~/.dmaker_launcher_history
...
check_running_electron() {
    ...
    if [ "$IS_WINDOWS" = true ]; then
        # Windows: look for electron.exe or DMaker.exe
        electron_pids=$(tasklist 2>/dev/null | grep -iE "electron|dmaker" | awk '{print $2}' | tr '\n' ' ' || true)
    else
        # Unix: look for electron or DMaker processes
        electron_pids=$(pgrep -f "electron.*dmaker|DMaker" 2>/dev/null | tr '\n' ' ' || true)
    fi
            ...
            case "$choice" in
                [kK]|[kK][iI][lL][lL])
                    ...
                    if [ "$IS_WINDOWS" = true ]; then
                        ...
                        taskkill //F //IM "DMaker.exe" 2>/dev/null || true
                    else
                        pkill -f "electron.*dmaker" 2>/dev/null || true
                        pkill -f "DMaker" 2>/dev/null || true
                    fi
                    sleep 1
